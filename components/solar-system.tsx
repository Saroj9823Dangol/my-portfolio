...
const Sun = memo(function Sun({ onClick }: { onClick: () => void }) {
  ...
  useFrame((state) => {
    ...
    if (coronaRef.current) {
      const scale =
        SUN_CONFIG.coronaScale + Math.sin(time * SUN_CONFIG.pulseSpeed) * 0.15;
      coronaRef.current.scale.setScalar(scale);
      coronaRef.current.rotation.z += 0.001;
    }
    if (outerCoronaRef.current) {
      const scale =
        SUN_CONFIG.coronaScale * 1.3 +
        Math.sin(time * SUN_CONFIG.pulseSpeed * 0.7) * 0.2;
    ...
    <group ref={prominencesRef}>
      {prominences.map((p, i) => (
        <mesh
          key={i}
          position={[
            Math.cos(p.angle) * SUN_CONFIG.radius * 1.05,
            0,
            Math.sin(p.angle) * SUN_CONFIG.radius * 1.05,
          ]}
...
const Planet = memo(function Planet({
    ...
    if (config.surfaceType === "gas-giant") {
      ...
      for (let y = 0; y < size; y++) {
        ...
        for (let x = 0; x < size; x += 4) {
          ...
          ctx.fillStyle =
            bandIndex % 2 === 0 ? config.secondaryColor : config.baseColor;
      ...
      for (let i = 0; i < 100; i++) {
        ...
        craterGradient.addColorStop(
          0.6,
          config.tertiaryColor || config.secondaryColor
        );
      ...
      for (let i = 0; i < 20; i++) {
        ctx.globalAlpha = 0.2;
        ctx.fillStyle =
          Math.random() > 0.5 ? config.secondaryColor : config.baseColor;
        ctx.beginPath();
        ctx.arc(
          Math.random() * size,
          Math.random() * size,
  ...
  return (
    <group ref={groupRef} rotation={[0, 0, config.axialTilt]}>
      ...
      {/* Saturn's rings */}
      {config.hasRings && config.ringColors && (
        <SaturnRings size={config.size} colors={config.ringColors} />
      )}
...
const SaturnRings = memo(function SaturnRings({
  size,
  colors,
}: {
  size: number;
...
const RealisticMoons = memo(function RealisticMoons({
  ...
  const moonsData = useMemo(() => {
    return Array.from({ length: count }, (_, i) => ({
      ...
      color: `hsl(${30 + Math.random() * 20}, ${5 + Math.random() * 10}%, ${
        50 + Math.random() * 20
      }%)`,
    }));
  }, [count, planetSize]);
  ...
  useFrame((state) => {
    if (meshRef.current && !isPaused) {
      ...
      meshRef.current.position.y =
        Math.sin(angle * 2) * inclination * orbitRadius * 0.2;
...
export function SolarSystem({
  ...
  return (
    <div className="fixed inset-0 z-10">
      ...
      >
        <PerspectiveCamera makeDefault position={[0, 9, 21]} fov={55} />
        <OrbitControls
          enablePan={false}
          enableZoom={true}
          minDistance={5}
